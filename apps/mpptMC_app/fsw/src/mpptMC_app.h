#ifndef _MPPTMC_APP_H_
#define _MPPTMC_APP_H_

#include "cfe.h"
#include "cfe_msg.h"
#include "cfe_core_api_base_msgids.h"

#include "mpptMC_app_config.h"
#include "mpptMC_app_msgids.h"
#include "mpptMC_app_msg.h"
#include "mpptMC_app_extern_typedefs.h"
#include "mpptMC_app_events.h"
#include "mpptMC_app_tbldefs.h"
#include "mpptMC_app_dispatch.h"
#include "mpptMC_app_cmds.h"

/*************************************************************************/
/*
** Telemetry Data
*/

/*************************************************************************/
/*
** Main MpptMC App Data Structure
*/

typedef struct
{
    MPPTMC_HkPacket_t    HkPacket; /**< \brief HK Housekeeping Packet */

    CFE_SB_PipeId_t         CmdPipe;    /**< \brief Pipe Id for HK command pipe */

    uint8                   CmdCounter;
    uint8                   ErrCounter;
    uint8                   MpptMissCount;                     /**< \brief Cycles elapsed without an MPPT HK response */
    uint16                  Readings[MPPTMC_NUM_READINGS];     /**< \brief Cached MPPT telemetry readings            */
    bool                    IsImaging;                         /**< \brief Cached PayloadMC imaging-mode broadcast   */

    CFE_ES_MemHandle_t      MemPoolHandle; /**< \brief HK mempool handle for output pkts */
    uint32                  RunStatus;     /**< \brief HK App run status */

    CFE_TBL_Handle_t            ConfigTableHandle;    /**< \brief Copy Table handle */
    MPPTMC_ConfigTbl_entry_t * ConfigTablePtr;    /**< \brief Ptr to copy table entry */

    uint8 MemPoolBuffer[MPPTMC_NUM_BYTES_IN_MEM_POOL]; /**< \brief HK mempool buffer */
} MPPTMC_AppData_t;

extern MPPTMC_AppData_t MPPTMC_AppData;

void MPPTMC_appMain(void);
/**
 * @brief Main function for the MpptMC application
 *
 * This function initializes the application, processes commands, and handles telemetry.
 * It runs in a loop until the application is terminated, either by an error or entire shutdown.
 */

CFE_Status_t MPPTMC_appInit(void);
/**
 * @brief Initializes the MpptMC application
 *
 * This function sets up the application, registers events, creates software bus pipes, subscribes to messages, and initializes tables.
 * @return CFE_Status_t Returns CFE_SUCCESS on successful initialization, or an error code on failure.
 */

CFE_Status_t MPPTMC_appTableInit(CFE_TBL_Handle_t *TblHandlePtr, MPPTMC_ConfigTbl_entry_t **TblPtr);
/**
 * @brief Initializes the configuration table for the MpptMC application
 *
 * This function registers the configuration table, gets its address, and initializes it.
 * @param TblHandlePtr Pointer to the table handle
 * @param TblPtr Pointer to the table pointer
 * @return CFE_Status_t Returns CFE_SUCCESS on successful initialization, or an error code on failure.
 */

CFE_Status_t MPPTMC_appTableReload(CFE_TBL_Handle_t *TblHandlePtr, MPPTMC_ConfigTbl_entry_t **TblPtr);
/**
 * @brief Reloads the configuration table for the MpptMC application
 *
 * This function releases the current table address, reinitializes the table, and gets its address again.
 * @param TblHandlePtr Pointer to the table handle
 * @param TblPtr Pointer to the table pointer
 * @return CFE_Status_t Returns CFE_SUCCESS on successful reload, or an error code on failure.
 */

CFE_Status_t MPPTMC_appResetHkData(void);
/**
 * @brief Resets the housekeeping data for the MpptMC application
 *
 * This function resets the command and error counters and cached telemetry readings to their initial values.
 * @return CFE_Status_t Returns CFE_SUCCESS on successful reset.
 */

CFE_Status_t MPPTMC_appPrepareHkPacket(void);
/**
 * @brief Prepares the housekeeping packet for the MpptMC application
 *
 * This function populates the housekeeping packet with the current command and error counters and cached MPPT readings.
 * @return CFE_Status_t Returns CFE_SUCCESS on successful preparation, or an error code on failure.
 */


#endif /* _MPPTMC_APP_H_ */