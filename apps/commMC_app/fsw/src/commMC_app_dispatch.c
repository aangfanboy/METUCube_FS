...
void COMMMC_appTaskPipe(const CFE_SB_Buffer_t *SBBufPtr)
{
    ...
    CFE_SB_MsgId_t CommandTaskId = CFE_SB_INVALID_MSG_ID;
    ...
    switch (CFE_SB_MsgIdToValue(MsgId))
    {
        case COMMMC_CMD_MID:
            ...
            if (CFE_SB_MsgId_Equal(CFE_SB_MsgIdFromValue(CommandTaskId), COMMMC_APP_COMMAND_TASK_ID_SEND_MINIMAL_TM_TO_GROUND))
            {
                CFE_EVS_SendEvent(COMMMC_SEND_MINIMAL_TM_EID, CFE_EVS_EventType_INFORMATION,
                    "COMMMC: Sent minimal telemetry to ground");
            }
            else if (CFE_SB_MsgId_Equal(CFE_SB_MsgIdFromValue(CommandTaskId), COMMMC_APP_COMMAND_TASK_ID_SEND_MAX))
            {
                CFE_EVS_SendEvent(COMMMC_SEND_MINIMAL_TM_EID, CFE_EVS_EventType_INFORMATION,
